...
/** Format integer → Rupiah string with dots */
function fmtN(n) {
...
function setStatus(s) {
  ...
  document.querySelectorAll('.toggle-group')[0]
    ...
    .forEach((b, i) => b.classList.toggle('active', ['TK', 'K', 'K/I'][i] === s));
}
...
function setNpwp(v) {
  ...
  document.querySelectorAll('.toggle-group')[1]
    .querySelectorAll('.toggle-btn')
    .forEach((b, i) => b.classList.toggle('active', i === (v ? 0 : 1)));
}
...
function hitungPPh() {
  ...
  // 2. Penghasilan bruto tahunan
  const bruto = (gajiPerBulan + tunjanganBulan) * 12 + bonusTahunan;

  // 3. Pengurang (PER-16/PJ/2016)
  const biayaJabatan = Math.min(bruto * 0.05, 6000000);
  const bpjsKesThn   = gajiPerBulan * (pctBpjsKes / 100) * 12;
  const bpjsTKThn    = gajiPerBulan * (pctBpjsTK  / 100) * 12;
  const pensiunThn   = pensiunBulan * 12;
  const totalPoton   = biayaJabatan + bpjsKesThn + bpjsTKThn + pensiunThn;

  // 4. Neto & PKP
  const neto     = bruto - totalPoton;
  ...
  renderPkpTable(gajiPerBulan, tunjanganBulan, bonusTahunan, bruto,
    biayaJabatan, bpjsKesThn, bpjsTKThn, pensiunThn,
    neto, ptkpKey, ptkp, pkpBulat, pajakTahunan);
  renderDonut(takehome * 12, pajakTahunan, bpjsKesThn + bpjsTKThn + pensiunThn);
  renderBracket(layers);
  renderPtkpTable(ptkpKey, ptkp);
  renderRekap(gajiPerBulan, tunjanganBulan, bonusTahunan,
    pajakTahunan, bpjsKesThn, bpjsTKThn, pensiunThn, takehome);

  // Tampilkan hasil
  document.getElementById('emptyState').style.display = 'none';
  const rc = document.getElementById('resultContent');
  rc.style.display = 'block';
  rc.classList.remove('slide-up');
  void rc.offsetWidth; // force reflow
  rc.classList.add('slide-up');
  setRtab('ringkasan');
...
function renderSummaryStrip(pajakBln, takehome, pajakThn, pkp) {
  document.getElementById('summaryStrip').innerHTML = `
    <div class="strip-cell">
      <div class="strip-lbl">PPh 21 / Bulan</div>
      <div class="strip-val red">${fmtRp(pajakBln)}</div>
      <div class="strip-sub">Potongan pajak</div>
    </div>
    <div class="strip-cell">
      <div class="strip-lbl">Gaji Bersih / Bulan</div>
      <div class="strip-val teal">${fmtRp(takehome)}</div>
      <div class="strip-sub">Setelah semua potongan</div>
    </div>
    <div class="strip-cell">
      <div class="strip-lbl">Total Pajak / Tahun</div>
      <div class="strip-val">${fmtRp(pajakThn)}</div>
      <div class="strip-sub">PKP: ${fmtRp(pkp)}</div>
    </div>
  `;
}
...
function renderPkpTable(gajiBln, tunjBln, bonus, bruto, biayaJab,
  ...
  document.getElementById('pkpTable').innerHTML = `
    <tr>
      <td class="label-cell"><span class="plus-sign">+</span>Gaji Pokok (12 bln)</td>
      <td>${fmtRpFull(gajiBln * 12)}</td>
    </tr>
    ...
    <tr>
      <td class="label-cell" style="font-weight:600;color:rgba(255,255,255,0.8)">Penghasilan Bruto</td>
      <td style="font-weight:700;color:white">${fmtRpFull(bruto)}</td>
    </tr>
    ...
    <tr class="total-row tax-total"><td>PPh 21 Setahun</td><td>${fmtRpFull(pajak)}</td></tr>
  `;
}

function renderDonut(takehomeThn, pajakThn, iuranThn) {
  ...
  chartD = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [takehomeThn, pajakThn, iuranThn],
        backgroundColor: ['#00b4a0', '#ef4444', '#f59e0b'],
        borderWidth: 0,
        hoverOffset: 10,
      }]
    },
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: c => ' ' + fmtRpFull(c.raw) } },
      },
    },
  });
}

function renderBracket(layers) {
  document.getElementById('bracketBody').innerHTML = layers.map(l => `
    <tr class="${l.pkpLayer > 0 ? 'row-active' : ''}">
      <td>${l.label}</td>
      <td>${(l.rate * 100).toFixed(0)}%${l.pkpLayer > 0 ? '<span class="badge-active">aktif</span>' : ''}</td>
      <td>${fmtRpFull(l.pkpLayer)}</td>
      <td>${fmtRpFull(l.pajak)}</td>
    </tr>
  `).join('');
}

function renderPtkpTable(activeKey, ptkpVal) {
  const rows = Object.entries(PTKP).map(([k, v]) => `
    <tr class="${k === activeKey ? 'ptkp-active' : ''}">
      <td>${PTKP_LABELS[k]}</td>
      <td>${fmtRpFull(v)}</td>
    </tr>
  `).join('');
  ...
  document.getElementById('ptkpTable').innerHTML = `
    <thead>
      <tr style="background:rgba(0,0,0,0.3)">
        <th style="padding:9px 12px;color:rgba(255,255,255,0.4);font-size:10px;letter-spacing:.07em;text-transform:uppercase">Status PTKP</th>
        <th style="padding:9px 12px;color:rgba(255,255,255,0.4);font-size:10px;letter-spacing:.07em;text-transform:uppercase;text-align:right">Jumlah PTKP</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  `;
  document.getElementById('ptkpSaya').textContent = fmtRpFull(ptkpVal) + ' (' + activeKey + ')';
}

function renderRekap(gajiBln, tunjBln, bonus, pajakThn, bpjsKes, bpjsTK, pensiun, takehome) {
  const bulanLabels = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'];
  const brutoPerBln = gajiBln + tunjBln;
  const pajakPerBln = pajakThn / 12;
  ...
  chartB = new Chart(ctx, {
    ...
    data: {
      labels: bulanLabels,
      datasets: [
        { label:'Gaji Bruto',    data: bulanLabels.map(() => brutoPerBln), backgroundColor:'rgba(0,180,160,0.25)', borderRadius:4 },
        { label:'PPh 21',        data: bulanLabels.map(() => pajakPerBln), backgroundColor:'#ef4444',             borderRadius:4 },
        { label:'Take Home Pay', data: bulanLabels.map(() => takehome),    backgroundColor:'#00b4a0',             borderRadius:4 },
      ],
    },
    options: {
      responsive: true,
      ...
      plugins: {
        legend: {
          labels: { font:{ family:'Plus Jakarta Sans', size:11 }, color:'rgba(255,255,255,0.5)', boxWidth:10, padding:14 },
        },
        tooltip: { callbacks: { label: c => ' ' + c.dataset.label + ': ' + fmtRpFull(c.raw) } },
      },
      scales: {
        y: {
          grid: { color:'rgba(255,255,255,0.05)' },
          ticks: { callback: v => fmtRp(v), font:{ family:'Plus Jakarta Sans', size:10 }, color:'rgba(255,255,255,0.3)' },
        },
        x: {
          grid: { display: false },
          ticks: { font:{ family:'Plus Jakarta Sans', size:10 }, color:'rgba(255,255,255,0.3)' },
        },
      },
    },
  });
  ...
  document.getElementById('rekap12Table').innerHTML = `
    <tr><td class="label-cell">Gaji Bruto Setahun</td><td>${fmtRpFull((gajiBln + tunjBln) * 12 + bonus)}</td></tr>
    <tr class="deduct"><td class="label-cell">Total PPh 21 Setahun</td><td>−${fmtRpFull(pajakThn)}</td></tr>
    <tr class="deduct"><td class="label-cell">Total BPJS &amp; Iuran Setahun</td><td>−${fmtRpFull(bpjsKes + bpjsTK + pensiun)}</td></tr>
    <tr class="total-row"><td>Take Home Pay Setahun</td><td style="color:var(--teal-light)">${fmtRpFull(takehome * 12)}</td></tr>
  `;
}
...
function toggleTip(e, id) {
  ...
  // Tutup semua tooltip
  document.querySelectorAll('.tip-bubble').forEach(b => b.classList.remove('show'));
  ...
  if (!isOpen) {
    ...
    // Posisi cerdas: flip ke kiri jika terlalu dekat tepi kanan (desktop)
...
// Inisialisasi toggle group status pernikahan
document.querySelectorAll('.toggle-group')[0]
  .querySelectorAll('.toggle-btn')
  .forEach((b, i) => { b.onclick = () => setStatus(['TK', 'K', 'K/I'][i]); });

// Inisialisasi toggle group NPWP
document.querySelectorAll('.toggle-group')[1]
  .querySelectorAll('.toggle-btn')
